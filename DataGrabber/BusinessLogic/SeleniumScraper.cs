...
namespace DataGrabber.BusinessLogic
{
    public sealed class SeleniumScraper : IScraper
    {
        ...
        public List<WebScrapeOutput> ProcessRequest(WebScrapeInput input)
        {
            ...
            {
                Logger.Write("Exception in ProcessRequest -- ScrapeProductsList -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
        ...
        private void ProcessRequestFor(ChromeDriver driver, WebScrapeInput input, ref List<WebScrapeOutput> result)
        {
            ...
            {
                Logger.Write("Exception in ProcessRequestForSingleURL -- ScrapeProductsListInput -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
        ...
        private List<WebScrapeOutput> GetContent(ChromeDriver driver, HtmlDocument htmlDoc, WebScrapeInput input, int pageNumber)
        {
            ...
            try
            {
                ...
                if (input.FetchDataFromIFrame)
                {
                    ...
                    {
                        Logger.Write($"ERROR: Unable to find iframe element for {input.Website.Name} in GetContent-- ScrapeProductsList -> DataGrabber.");
                    }
                    else
                    {
                ...
                if (parentContainers == null)
                {
                    ...
                    {
                        Logger.Write($"Info: Retrying count: {counter}; Parent Container not found for WebsiteName: {input.Website.Name}; URL: {input.Website.URL} in ProcessRequest -- ScrapeProductsList -> DataGrabber. ");
                ...
                if (parentContainers == null)
                {
                    Logger.Write($"ERROR: Parent Container not found for WebsiteName: {input.Website.Name}; URL: {input.Website.URL} in ProcessRequest -- ScrapeProductsList -> DataGrabber.");
                }
                else
                {
                    ...
                    foreach (HtmlNode parentContainer in parentContainers)
                    {
                        ...
                        {
                            Logger.Write($"ERROR: Product Container not found for WebsiteName: {input.Website.Name}; URL: {input.Website.URL}; in ProcessRequest -- ScrapeProductsList -> DataGrabber.");
                        }
                        else
                        {
            ...
            {
                Logger.Write("Exception in GetContent -- ScrapeProductsList -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
        ...
        private string GetHTMLContent(ChromeDriver driver, HtmlNode container, HtmlNode bodyContainer, ElementMapping mapping, string baseURL)
        {
            ...
            try
            {
                ...
                if (objNode != null)
                {
                    ...
                    if (mapping.GetChildElement)
                    {
                        ...
                        if (childNodes == null)
                        {
                            Logger.Write($"ERROR: Child Node Path {mapping.ChildNode.XPath} not found in GetHTMLContent -- ScrapeProductsList -> DataGrabber. ");
                        }
                        else
                        {
            ...
            {
                Logger.Write("Exception in GetHTMLContent -- ScrapeProductsList -> DataGrabber. Message: " + ex.Message);
                return result;
            }
            finally
            {
        ...
        public void ProcessDetailRequest(WebScrapeInput input, ref List<WebScrapeOutput> result)
        {
            ...
            {
                Logger.Write("Exception in ProcessDetailRequest -- ScrapeProductsList -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
        ...
        private void AddUniqueIdentifier(ref List<WebScrapeOutput> result)
        {
            ...
            {
                Logger.Write("Exception in AddUniqueIdentifier -- ScrapeProductsListInput -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
        ...
        private void FormatDataBasedOnScrape(List<WebScrapeOutput> result, string TargetNameForInputURL, ref WebScrapeInput detailInput)
        {
            ...
            {
                Logger.Write("Exception in FormatDataBasedOnScrape -- ScrapeProductsListInput -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
        ...
        private bool CheckIfMorePagesAreAvailable(ChromeDriver driver, WebScrapeInput input, int pageNumber)
        {
                    ...
                    case PaginationType.LoadOnClick:
                        {
                            ...
                            {
                                Logger.Write("ERROR: Unable to find pagination container in CheckIfMorePagesAreAvailable -- ScrapeProductsList -> DataGrabber.");
                                return false;
                            }
                            else
                            {
                    ...
                    case PaginationType.LoadOnShowMore:
                        {
                            ...
                            {
                                Logger.Write("ERROR: Unable to find LoadMoreButton in CheckIfMorePagesAreAvailable -- ScrapeProductsList -> DataGrabber.");
                                return false;
                            }
                            else
                            {
            ...
            {
                Logger.Write("Exception in CheckIfMorePagesAreAvailable -- ScrapeProductsList -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
        ...
        private bool LoadPageBasedOnInput(ChromeDriver driver, WebScrapeInput data)
        {
            ...
            try
            {
                ...
                if (data.HasSearchButton)
                {
                    ...
                    {
                        Logger.Write($"ERROR: Unable to find search button for {data.Website.Name} in LoadPageBasedOnInput-- ScrapeProductsList -> DataGrabber.");
                        result = false;
                    }
                    else
                    {
            ...
            {
                Logger.Write("Exception in Main -- LoadPageBasedOnInput -> DataGrabber. Message: " + ex.Message);
                result = false;
            }
            finally
            {
        ...
        private void SetBrowserOptions(WebScrapeInput input, out ChromeDriver driver)
        {
            ...
            catch (Exception ex)
            {
                ...
                Logger.Write("Exception in SetBrowserOptions -- ScrapeProductsListInput -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
        ...
        private List<OpenQA.Selenium.Cookie> SetAllBrowserCookie(ChromeDriver driver, string baseURL, string domain)
        {
            ...
            try
            {
                ...
                //    catch (Exception ex)
                //    {
                //        Logger.Write("Exception in PROCESS KILL FAILED SetAllBrowserCookie -- ScrapeProductsListInput -> DataGrabber. Message: " + ex.Message);
                //    }
                //    finally
                //    {
                //        Thread.Sleep(5000);
            ...
            {
                Logger.Write("Exception in SetAllBrowserCookie -- ScrapeProductsListInput -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
                try
                ...
                {
                    Logger.Write("Exception in finally block - SetAllBrowserCookie -- ScrapeProductsListInput -> DataGrabber. Message: " + ex.Message);
                }
            }
            return cookies;
        }
        ...
        private void SetFieldsBasedOnInput(ChromeDriver driver, WebDriverWait wait, WebScrapeInput data)
        {
            try
            {
                if (data.HasResetSearchButton)
                {
                    ...
                    {
                        Logger.Write($"ERROR: Unable to find both reset & search button for {data.Website.Name} in SetFieldsBasedOnInput-- ScrapeProductsList -> DataGrabber.");
                        return;
                    }
                    else if (!driver.IsElementPresent(resetSearchElemPath))
                    {
                        Logger.Write($"ERROR: Unable to find reset search button for {data.Website.Name} in SetFieldsBasedOnInput-- ScrapeProductsList -> DataGrabber.");
                    }
                    else
                    {
                        // click on search button
                ...
                foreach (var inp in data.Website.InputInfo)
                {
                    ...
                    {
                        Logger.Write($"ERROR: Input field missing in SetFieldsBasedOnInput -- LoadPageBasedOnInput -> DataGrabber." +
                    ...
                    switch (inp.TargetType.ToLower())
                    {
                        ...
                        case "auto-suggest":
                            ...
                            {
                                Logger.Write($"ERROR: Unable to find partner element in SetFieldsBasedOnInput -- LoadPageBasedOnInput -> DataGrabber." +
            ...
            {
                Logger.Write("Exception in SetFieldsBasedOnInput -- ScrapeProductsListInput -> DataGrabber. Message: " + ex.Message);
            }
            finally
            {
