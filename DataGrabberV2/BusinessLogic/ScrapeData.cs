...
using DataGrabberV2.BusinessLogic.Factory;
...
using System.Web;
using CsvHelper;
using DataGrabberV2.Models.RestLibrary.ConfigTool;
    ...
    public class ScrapeData
    {
        ...
        public void Main()
        {
            try
            {
                ...
                {
                    string UpdateScrapeRequestURL = string.Format(ConfigFields.Api_UpdateScrapeRequest, inp.ScrapeID);
                    var scrapeReq = new BasicScrapeRequestDetail() { ScrapeID = inp.ScrapeID };

                    // update the req as started
                    var updateResp = RestHelper.Instance.ServiceFactory(UpdateScrapeRequestURL, HttpVerbType.PUT, scrapeReq.SetDataForStart());
                    ...
                    {
                        // update the req as complete
                        updateResp = RestHelper.Instance.ServiceFactory(UpdateScrapeRequestURL, HttpVerbType.PUT, scrapeReq.SetDataForComplete(response.Message));
                    }
                    else
                    {
                        // update the req as failed
                        updateResp = RestHelper.Instance.ServiceFactory(UpdateScrapeRequestURL, HttpVerbType.PUT, scrapeReq.SetDataForFail(response.Message));
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                Logger.Write("Exception in Main -- ScrapeProductsList -> DataGrabberV2. Message: " + ex.Message);
        ...
        private DbResponse SaveDataExcelFile(List<WebScrapeOutput> output)
        {
            ...
            catch (Exception ex)
            {
                ...
                Logger.Write("Exception in SaveDataFile -- ScrapeProductsList -> DataGrabberV2. Message: " + ex.Message);
            }
            finally
            {
        ...
        private List<WebScrapeInput> FormatData(string json)
        {
            ...
            {
                Logger.Write("Exception in FormatData -- ScrapeProductsList -> DataGrabberV2. Message: " + ex.Message);
            }
            finally
            {
        ...
        private WebScrapeInput FormatUserInputData(WebScrapeUserInput data)
        {
            ...
            {
                Logger.Write("Exception in FormatUserInputData -- ScrapeProductsListInput -> DataGrabberV2. Message: " + ex.Message);
            }
            finally
            {
        ...
        private void GetDataFromDB()
        {
            try
            {
                List<WebScrapeUserInput> inputs = new List<WebScrapeUserInput>();

                // get all requests
                var allRequestsResp = RestHelper.Instance.ServiceFactory(ConfigFields.Api_GetAllScrapeRequest, HttpVerbType.GET);

                if (allRequestsResp.StatusCode == HttpStatusCode.OK)
                {
                    var allRequests = JsonConvert.DeserializeObject<List<BasicScrapeRequestDetail>>(allRequestsResp.Message);

                    foreach (var scrapeReq in allRequests)
                    {
                        string GetConfigurationDetailURL = string.Format(ConfigFields.Api_GetConfigurationDetail, scrapeReq.ConfigGUID);

                        // get config detail
                        var configResp = RestHelper.Instance.ServiceFactory(GetConfigurationDetailURL, HttpVerbType.GET);

                        if (configResp.StatusCode == HttpStatusCode.OK)
                        {
                            var configDetail = JsonConvert.DeserializeObject<FullConfigurationDetail>(configResp.Message);

                            // map data from config to webscrapeInput class
                            var input = MapDataFromDbToWebScrapeInput(configDetail);

                            if (input != null)
                            {
                                input.ScrapeID = scrapeReq.ScrapeID;
                                inputs.Add(input);
                            }
                ...
                if (inputs.Count > 0)
                {
                    string inputFile = JsonConvert.SerializeObject(inputs,
                            Newtonsoft.Json.Formatting.None,
                            new JsonSerializerSettings
                            {
                                NullValueHandling = NullValueHandling.Ignore
                            });
                    File.WriteAllText(ConfigFields.InputFile, inputFile);
                }

            }
            catch (Exception ex)
            {
                Logger.Write("Exception in GetDataFromDB -> DataGrabberV2. Message: " + ex.Message);
            }
            finally
            {
        ...
        private WebScrapeUserInput MapDataFromDbToWebScrapeInput(FullConfigurationDetail configDetail)
        {
            var result = new WebScrapeUserInput();

            try
            {
                var siteConfig = configDetail.siteConfiguration;
                var websiteDetail = configDetail.websiteDetail;
                var productDetail = configDetail.productDetail;
                var paginationDetail = configDetail.paginationDetail;
                var mailingInfo = configDetail.mailingInformation;

                result.ScrapeID = configDetail.scrapeID;
                result.WebsiteConfigurationName = siteConfig.websiteConfigurationName;
                //result.WebsiteInfo = new List<WebsiteInfo>
                result.shouldRotateProxyIP = siteConfig.shouldRotateProxy.SetYesOrNo();
                result.shouldDisableJavaScript = siteConfig.shouldDisableJavaScript.SetYesOrNo();

                result.WaitingTimeAfterPageLoad = string.IsNullOrEmpty(siteConfig.waitingTimeAfterPageLoad) ? 0 : int.Parse(siteConfig.waitingTimeAfterPageLoad);
                //result.WaitingTimeAfterPageClick = int.Parse(siteConfig.waitingTimeAfterPageLoad);

                result.DefaultWebsiteConfiguration = new WebsiteInfo()
                {
                    //WebsiteName = websiteDetail.websiteNamePrefix,
                    WebsiteURL = websiteDetail.websiteURL,
                    WebsiteURLs = websiteDetail.websiteURLs.ToArray(),
                    InputInfo = websiteDetail.inputFields.Select(v => new InputTargetElement()
                    {
                        TargetName = v.fieldName,
                        TargetType = v.targetType,
                        ElementIdentifier = v.fieldPath,
                        ElementType = v.fieldPathType,
                        WaitingTimeAfterElementChange = string.IsNullOrEmpty(v.waitingTimeAfterElementChange) ? 0 : int.Parse(v.waitingTimeAfterElementChange),
                        //PartnerElement = 
                        //TargetValue = v.
                    }).ToList(),
                    WebsiteNamePrefix = websiteDetail.websiteNamePrefix,
                    //TargetName = websiteDetail.ta
                    //InputValues
                    webScrapeType = websiteDetail.webscrapeType,
                    //WebsiteURLsWithBody
                    //WebsitePatternConfig
                };
                result.hasSearchButton = websiteDetail.doesHaveSearchButton.SetYesOrNo();
                result.SearchButton = new Element()
                {
                    ElementType = websiteDetail.searchButtonPathType,
                    ElementIdentifier = websiteDetail.searchButton
                };
                result.hasResetSearchButton = websiteDetail.doesHaveResetButton.SetYesOrNo();
                result.ResetSearchButton = new Element()
                {
                    ElementType = websiteDetail.resetButtonPathType,
                    ElementIdentifier = websiteDetail.resetButton
                };

                result.ParentContainer = new Element()
                {
                    ElementType = productDetail.overallContainerPathType,
                    ElementIdentifier = productDetail.overallContainer
                };
                result.Container = new Element()
                {
                    ElementType = productDetail.productContainerPathType,
                    ElementIdentifier = productDetail.productContainer
                };
                result.ProductDetails = productDetail.fields.Select(v => new TargetElement
                {
                    TargetName = v.fieldName,
                    ElementIdentifier = v.fieldPath,
                    ElementType = v.fieldPathType,
                    AttributeName = v.attributeName,
                    RemoveText = v.removeText,
                    //getChildElement = 
                    //OnlyCheckIfElementExists
                    shouldCheckElemInBody = v.shouldCheckElementInBody.SetYesOrNo()
                }).ToList();
                // fetchDataFromIFrame
                // IFrameElement

                result.pagingType = paginationDetail.pagingType;
                result.Pagination = new Element()
                {
                    ElementType = paginationDetail.paginationContainerPathType,
                    ElementIdentifier = paginationDetail.paginationContainer
                };
                result.hasNextButton = paginationDetail.doesHaveNextButton.SetYesOrNo();
                result.NextPaginationButton = new Element()
                {
                    ElementType = paginationDetail.nextButtonPathType,
                    ElementIdentifier = paginationDetail.nextButton
                };
                result.ActivePageClass = paginationDetail.activeClassForCurrentPage;
                result.DisabledPageClass = paginationDetail.disabledClassForLastPage;
                //result.LoadMoreButton

                result.shouldLimitPaging = paginationDetail.shouldLimitPaging.SetYesOrNo();
                result.PagingLimit = string.IsNullOrEmpty(paginationDetail.pagingLimit) ? 0 : int.Parse(paginationDetail.pagingLimit);
                //OutputSheetNumber
                //shouldFetchDataFromDetailsPage
                //TargetNameForInputURL
                //DetailedInformationPage

                result.shouldSendMailOfOutputData = mailingInfo.shouldSendMail.SetYesOrNo();
                result.MailInfo = new MailInformation()
                {
                    MailToAddress = mailingInfo.mailToAddress,
                    MailCCAddress = mailingInfo.mailCCAddress,
                    MailBCCAddress = mailingInfo.mailBCCAddress,
                };
            }
            catch (Exception ex)
            {
                result = null;
                Logger.Write("Exception in MapDataFromDbToWebScrapeInput -> DataGrabberV2. Message: " + ex.Message);
            }
            finally
            {
        ...
        private string GetNewCookieToken(string newCookieToken)
        {
            ...
            try
            {
                ...
                {
                    Logger.Write($"ERROR: {resp.StatusCode.ToString()} in GetNewCookieToken -- ScrapeProductsList -> DataGrabberV2. Message: {resp.Message}");
                }
            }
            catch (Exception ex)
            {
                Logger.Write("Exception in GetNewCookieToken -- ScrapeProductsList -> DataGrabberV2. Message: " + ex.Message);
        ...
        private string RefreshNewCookieToken(string oldCookieToken)
        {
            ...
            try
            {
                ...
                {
                    Logger.Write($"ERROR: {resp.StatusCode.ToString()} in RefreshNewCookieToken -- ScrapeProductsList -> DataGrabberV2. Message: {resp.Message}");
                }

            }
            catch (Exception ex)
            {
                Logger.Write("Exception in RefreshNewCookieToken -- ScrapeProductsList -> DataGrabberV2. Message: " + ex.Message);
            }
            finally
            {
...
//{
//    Logger.Write($"ERROR: Data not refreshed on clicking search button for {data.Website.Name} in LoadPageBasedOnInput-- ScrapeProductsList -> DataGrabberV2.");
//}
